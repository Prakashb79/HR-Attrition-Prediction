...
if st.button("📥 Generate PDF Report"):
    ...
    try:
        ...
        report_path = "employee_report.pdf"
        ...
        title = Paragraph(
            "HR Attrition Prediction Report",
            styles['Title']
        )
        ...
        details = f"""
        <b>Age:</b> {Age}<br/>
        <b>Monthly Income:</b> {MonthlyIncome}<br/>
        <b>Job Satisfaction:</b> {JobSatisfaction}<br/>
        <b>Work Life Balance:</b> {WorkLifeBalance}<br/>
        <b>Over Time:</b> {OverTime}<br/>
        <b>Total Working Years:</b> {TotalWorkingYears}<br/>
        <b>Years At Company:</b> {YearsAtCompany}<br/>
        <b>Performance Rating:</b> {PerformanceRating}<br/>
        """
        ...
        # Prediction Result
        result_text = f"""
        <b>Attrition Probability:</b> {probability*100:.2f}%<br/>
        """

        if probability > 0.35:

            result_text += """
            <b>Status:</b> Employee is likely to leave the company.
            """

        else:

            result_text += """
            <b>Status:</b> Employee is likely to stay in the company.
            """
